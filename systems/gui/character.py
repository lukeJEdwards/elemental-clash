...
def get_sprite(filename: str, active: bool):
    filenames = (
        f"{assetsDirs.ICONS}\\{filename}.png",
        f"{assetsDirs.UI.BORDERS}\\player-border.png",
        f"{assetsDirs.UI.BORDERS}\\player-border-background.png",
    )
...
class CharacterIcon(RenderObject, SelectableGuiObject):
    def __init__(self, pos: tuple[int, int], filename: str, call_back: callable) -> None:
