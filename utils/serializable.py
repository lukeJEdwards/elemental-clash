...
class Serializable:
    ...
    def get_serializable_items(self) -> ItemsView[str, object]: return {key: value for key, value in self.__dict__.items() if key[0] != '_'}.items()
    
    def serialize(self) -> dict: return {key: self.encode(value) for key, value in self.get_serializable_items()}
...
def json_dump(filename:str, data:dict|Serializable, **options:dict) -> dict:
    with open(filename, 'w', encoding=options.get('encoding', 'utf-8')) as file:
        ...
        json.dump(data.serialize() if isinstance(data, Serializable) else data, file, sort_keys=options.get('sort_keys', True), indent=options.get('indent', 2))
    file.close()
    return data
