...
class Serializable:
    ...
    def serialize(self) -> dict:
        return {key: encode(value) for key, value in self.get_serializable_items()}
...
def json_dump(filename: str, data: dict | Serializable, **options: dict) -> dict:
    with open(filename, "w", encoding=options.get("encoding", "utf-8")) as file:
        data = {
            key: value.serialize() if isinstance(value, Serializable) else encode(value) for key, value in data.items()
        }
        json.dump(
